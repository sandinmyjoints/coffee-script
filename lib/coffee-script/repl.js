...
(function() {
  var ACCESSOR, CoffeeScript, MAGICS, Module, REPL_PROMPT, REPL_PROMPT_CONTINUATION, REPL_PROMPT_MULTILINE, SIMPLEVAR, Script, autocomplete, backlog, completeAttribute, completeVariable, enableColours, error, getCompletions, inspect, magic, multilineMode, pipedInput, proc, readline, repl, run, shell, stdin, stdout,
  ...
  shell = function(cmd, cb) {
    return proc.exec(cmd, function(err, stdout, stderr) {
      ...
      return cb(null, stdout.toString().trim());
    });
  };

  MAGICS = {
    pwd: function(done) {
      return shell('pwd', done);
    },
    ls: function(done) {
      return shell('ls', done);
    }
  };
